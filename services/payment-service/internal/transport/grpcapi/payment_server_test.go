...
func (f fakePaymentCreator) CreatePayment(_ context.Context, _ string, _ int64, _ string) (*domain.Payment, error) {
...
func TestPaymentServer_CreatePayment_Success(t *testing.T) {
	...
	server := grpcapi.NewPaymentServer(fakePaymentCreator{payment: payment})
	...
	if resp.GetPayment().GetStatus() != string(domain.StatusPending) {
		t.Errorf("expected status %q, got %q", domain.StatusPending, resp.GetPayment().GetStatus())
	}
}
